...
This method lets you render a [Pug](https://pugjs.org) template in HTML format as a **PDF document**
...
const app = new Koa;
pug(app, {basedir: join(import.meta.dirname, "path/to/view/folder")});
...
The `pug()` function accepts a `browser` option which is an object that is directly passed
...
## Rendering options
The `ctx.renderPdf()` method supports the same options as [the `ctx.render()` method](RenderHtml.md) (i.e. `writeResponse`).
